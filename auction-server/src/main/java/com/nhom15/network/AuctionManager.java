...
/**
 * Quản lý lock riêng cho từng phiên đấu giá. Đảm bảo thread-safe khi nhiều client đặt giá đồng thời
 * vào cùng 1 phiên.
 */
public class AuctionManager {
  ...
  /**
   * Lấy hoặc tạo lock cho phiên auction_id. computeIfAbsent đảm bảo chỉ tạo 1 lock duy nhất dù
   * nhiều thread cùng gọi.
   */
  ...
  /**
   * Đặt giá có lock: 1. Lock tầng application (ReentrantLock) — ngăn race condition trong JVM 2.
   * Bên trong DAO dùng SELECT ... FOR UPDATE — ngăn race condition ở DB
   */
  ...
  /**
   * Xoá lock khi phiên đấu giá kết thúc để tránh leak memory. Gọi từ AuctionHandler sau khi
   * END_AUCTION thành công.
   */
