...
public class AuctionDAO {
    ...
    public int createAuction(int itemId, int sellerId, double startPrice,
        ...
        String sql = "INSERT INTO auction (item_id, seller_id, start_price, current_price, min_step, end_time) " +
                "VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public JsonArray getActiveAuctions(int limit) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) result.add(mapRowToJson(rs));
            }
        } catch (SQLException e) { e.printStackTrace(); }
        return result;
    }
    ...
    public JsonObject getAuctionById(int auctionId) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return mapRowToJson(rs);
            }
        } catch (SQLException e) { e.printStackTrace(); }
        return null;
    }
    ...
    public boolean placeBid(int auctionId, int bidderId, double amount) {
        ...
        try (Connection conn = DBConnection.getConnection()) {
            ...
            try {
                ...
                if (amount < curPrice + minStep) return false;
    ...
    private JsonObject mapRowToJson(ResultSet rs) throws SQLException {
        ...
        obj.addProperty("description",  rs.getString("description") != null ? rs.getString("description") : "");
        obj.addProperty("category",     rs.getString("category")    != null ? rs.getString("category")    : "");
        obj.addProperty("imagePath",    rs.getString("image_path")  != null ? rs.getString("image_path")  : "");
