 ...
 *
 * Mỗi subclass chỉ cần override buildRequest() để cung cấp
 * JsonObject tương ứng với action của mình.
 *
 * executeAsync() xử lý toàn bộ:
 *   - Gửi request trên background thread
 *   - Callback onSuccess / onError trên JavaFX thread (Platform.runLater)
 *   - Phân biệt lỗi kết nối (response null) với lỗi nghiệp vụ (status FAIL)
 */
public abstract class ServerCommand {
    ...
    /**
     * Build JsonObject gửi lên server.
     * Ví dụ:
     ...
     *
     * @param onSuccess callback nhận JsonObject response, chạy trên FX thread.
     *                  Được gọi kể cả khi status = "FAIL" — caller tự kiểm tra.
    ...
    /**
     * Lấy message từ response, trả về fallback nếu không có.
     * Dùng để hiển thị lỗi từ server mà không cần try-catch.
     */
