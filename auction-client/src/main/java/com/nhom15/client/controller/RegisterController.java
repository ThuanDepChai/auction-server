 ...
 * <p>
 * Không tự build JsonObject, không tự FXMLLoader, không tự lấy Stage. Dùng: CheckUsernameCommand +
 * RegisterCommand (gửi server), ViewManager (điều hướng), FormValidator (validate realtime).
 */
public class RegisterController {
  ...
  /**
   * Kiểm tra toàn bộ form trước khi submit. Trả về false nếu bất kỳ điều kiện nào không thoả.
   */
  private boolean validateAll() {
    ...
    if (lblEmailError.isVisible() || lblPasswordError.isVisible()) {
      return false;
    }
  ...
  private void loadBackground() {
    try {
      ...
      if (sharedBg.getParent() instanceof Pane p) {
        p.getChildren().remove(sharedBg);
      }
