...
public class ProfileController {
    ...
    @FXML private Label     lblAvatarInitial, lblDisplayName, lblRoleBadge;
    @FXML private Label     lblBalanceAmount, lblJoinDate;
    @FXML private Label     lblInfoStatus, lblPasswordStatus;
    @FXML private Label     lblOldPassError, lblNewPassError, lblConfirmPassError;
    ...
    private void handleSaveInfo(ActionEvent event) {
        ...
        if (!email.matches("^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$")) {
            showStatus(lblInfoStatus, "Email không hợp lệ!", false); return;
        }
    ...
    private void handleChangePassword(ActionEvent event) {
        ...
        boolean err = false;
        if (oldPass.isEmpty())
        { setLabel(lblOldPassError,     "Vui lòng nhập mật khẩu hiện tại!");    err = true; }
        if (!newPass.matches("^(?=.*[A-Za-z])(?=.*\\d).{6,}$"))
        { setLabel(lblNewPassError,     "Mật khẩu phải từ 6 ký tự, có chữ và số!"); err = true; }
        if (!newPass.equals(confirm))
        { setLabel(lblConfirmPassError, "Mật khẩu xác nhận không khớp!");       err = true; }
        if (err) return;
    ...
    @FXML private void handleBack(ActionEvent e)            { ViewManager.navigateTo(ViewManager.Views.HOME); }
    @FXML private void handleGoHome(ActionEvent e)          { ViewManager.navigateTo(ViewManager.Views.HOME); }
    @FXML private void handleSellerDashboard(ActionEvent e) { ViewManager.navigateTo(ViewManager.Views.SELLER_DASHBOARD); }
    @FXML private void handleMyOrders(ActionEvent e)        { showAlert("Tính năng đang phát triển!"); }
    @FXML private void handleLogout(ActionEvent e)          { SessionManager.logout(); ViewManager.navigateTo(ViewManager.Views.LOGIN); }
    ...
    private void applyRoleBadge(String role) {
        ...
        lblRoleBadge.setStyle(s + "-fx-background-radius:10;-fx-padding:3 12 3 12;-fx-font-weight:bold;");
    }

    private void showStatus(Label lbl, String msg, boolean ok) {
        lbl.setText(msg); lbl.setTextFill(ok ? Color.web("#4CAF50") : Color.web("#D96570"));
    }
    private void setLabel(Label lbl, String msg) { lbl.setText(msg); lbl.setVisible(true); }
    ...
    private String  safeGet(JsonObject o, String k)            { return safeGet(o, k, ""); }
    private String  safeGet(JsonObject o, String k, String def){ return o.has(k) && !o.get(k).isJsonNull() ? o.get(k).getAsString() : def; }
}