...
public class CropAvatarController {
  ...
  @FXML
  private Pane paneImageContainer;
  @FXML
  ...
  public void initImage(File file, Consumer<String> callback) {
    ...
    try {
      ...
      double initialScale =
          CROP_SIZE / Math.min(originalImage.getWidth(), originalImage.getHeight());
        if (initialScale > 1.0) {
            initialScale = 1.0; // Không phóng to nếu ảnh nhỏ hơn khung
        }
      sliderZoom.setValue(initialScale * 100.0 / 2.0); // Cập nhật slider

      imgSource.setTranslateX(
          (350.0 - originalImage.getWidth()) / 2.0); // paneImageContainer rộng 350
  ...
  void handleSave(ActionEvent event) {
    ...
    try {
      ...
      WritableImage croppedWritableImage = new WritableImage(reader, (int) viewportX,
          (int) viewportY, (int) viewportSize, (int) viewportSize);

      // 3. Chuyển WritableImage thành PNG mảng byte
      ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
      java.awt.image.BufferedImage bufferedImage = SwingFXUtils.fromFXImage(croppedWritableImage,
          null);
