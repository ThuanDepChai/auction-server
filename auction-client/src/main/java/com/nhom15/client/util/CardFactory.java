...
public class CardFactory {
    ...
    public static VBox buildProductCard(JsonObject item, Consumer<Integer> onClick) {
        ...
        Label lblPrice = createLabel(formatPrice(price) + "đ", "-fx-font-size: 16px; -fx-font-weight: bold; -fx-text-fill: #D96570;");
    ...
    public static VBox buildAuctionCard(JsonObject auction, List<Timeline> activeTimers, Consumer<Integer> onBidClick) {
        ...
        Label lblName = createLabel(name, "-fx-font-size: 13px; -fx-text-fill: #333333;");
        Label lblPrice = createLabel("Giá hiện tại: " + formatPrice(curPrice) + "đ", "-fx-font-size: 13px; -fx-font-weight: bold; -fx-text-fill: #4285F4;");

        Label lblCountdown = createLabel("⏰ --:--:--", "-fx-font-size: 12px; -fx-text-fill: #D96570; -fx-font-weight: bold;");
        ...
        btnBid.setStyle("-fx-background-color: linear-gradient(to right, #4285F4, #9B72CB); -fx-background-radius: 8; -fx-text-fill: white; -fx-font-weight: bold; -fx-cursor: hand; -fx-padding: 7 0 7 0;");
    ...
    private static VBox createBaseCard(String base64, double imgHeight) {
        ...
        card.setStyle("-fx-background-color: white; -fx-background-radius: 10; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.08), 10, 0, 0, 4); -fx-cursor: hand;");
        card.setOnMouseEntered(e -> card.setStyle(card.getStyle().replace("0.08", "0.18").replace("10, 0, 0, 4", "15, 0, 0, 6")));
        card.setOnMouseExited(e -> card.setStyle(card.getStyle().replace("0.18", "0.08").replace("15, 0, 0, 6", "10, 0, 0, 4")));
        ...
        try {
            if (!base64.isEmpty()) {
                ...
                if (!img.isError()) {
                    ...
                    iv.setFitWidth(210); iv.setFitHeight(imgHeight); iv.setPreserveRatio(true);
        ...
        Label placeholder = new Label("🖼"); placeholder.setStyle("-fx-font-size: 36px; -fx-text-fill: #CCCCCC;");
    ...
    private static Label createLabel(String text, String style) {
        Label lbl = new Label(text);
        lbl.setWrapText(true); lbl.setMaxWidth(186); lbl.setStyle(style);
        return lbl;
    }
        ...
        try {
            ...
            Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(1), e -> {
                ...
                if (now.isAfter(endTime)) {
                    label.setText("⏰ Đã kết thúc"); label.setStyle("-fx-font-size: 12px; -fx-text-fill: #888888;");
                    return;
                }
                ...
                label.setStyle(hours < 1 ? "-fx-font-size: 12px; -fx-text-fill: #D96570; -fx-font-weight: bold;" : "-fx-font-size: 12px; -fx-text-fill: #E8A838; -fx-font-weight: bold;");
            }));
    ...
    private static String formatPrice(String raw) {
        try { return String.format("%,d", Long.parseLong(raw.replaceAll("[^0-9]", ""))); } catch (Exception e) { return raw; }
    }
    ...
    public static VBox buildSellerItemCard(JsonObject item, Consumer<Integer> onDelete) {
        ...
        String imgBase64 = item.has("imageBase64") && !item.get("imageBase64").isJsonNull() ? item.get("imageBase64").getAsString() : "";
        int itemId = item.get("itemId").getAsInt();

        VBox card = new VBox(8);
        card.setPrefWidth(200);
        card.setStyle("-fx-background-color: white; -fx-background-radius: 10; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.07), 10, 0, 0, 4);");
        ...
        if (!imgBase64.isEmpty()) {
            try {
                ...
                iv.setFitWidth(200); iv.setFitHeight(140); iv.setPreserveRatio(true);
                imgPane.getChildren().add(iv);
            } catch (Exception e) { imgPane.getChildren().add(new Label("🖼")); }
        } else {
            Label ph = new Label("🖼"); ph.setStyle("-fx-font-size: 30px; -fx-text-fill: #CCCCCC;");
            imgPane.getChildren().add(ph);
        }

        VBox info = new VBox(4); info.setStyle("-fx-padding: 8 10 10 10;");
        Label lblName = new Label(name); lblName.setWrapText(true); lblName.setStyle("-fx-font-size: 12px; -fx-font-weight: bold;");
        Label lblPrice = new Label(price + "đ"); lblPrice.setStyle("-fx-font-size: 13px; -fx-text-fill: #D96570; -fx-font-weight: bold;");
        Label lblCat = new Label(category); lblCat.setStyle("-fx-font-size: 11px; -fx-text-fill: #888888;");

        String statusColor = "AVAILABLE".equals(status) ? "#E8F5E9:#27AE60" : "IN_AUCTION".equals(status) ? "#EEF2FF:#4285F4" : "#FCE4EC:#C62828";
        String[] sc = statusColor.split(":");
        Label lblStatus = new Label("AVAILABLE".equals(status) ? "Sẵn bán" : "IN_AUCTION".equals(status) ? "Đang đấu giá" : "Đã bán");
        lblStatus.setStyle("-fx-background-color: " + sc[0] + "; -fx-text-fill: " + sc[1] + "; -fx-background-radius: 8; -fx-padding: 2 8 2 8; -fx-font-size: 10px;");

        HBox actions = new HBox(5);
        if ("AVAILABLE".equals(status)) {
            Button btnDelete = new Button("🗑 Xóa");
            btnDelete.setStyle("-fx-background-color: #FCE4EC; -fx-text-fill: #C62828; -fx-background-radius: 8; -fx-cursor: hand; -fx-padding: 4 10 4 10;");
    ...
    public static HBox buildSellerAuctionRow(JsonObject auction, Consumer<Integer> onEnd) {
        ...
        Label lblName = new Label(name); lblName.setStyle("-fx-font-weight: bold; -fx-font-size: 13px;");
        Label lblPrice = new Label("Giá hiện tại: " + curPrice + "đ"); lblPrice.setStyle("-fx-font-size: 12px; -fx-text-fill: #4285F4;");
        Label lblEnd = new Label("Kết thúc: " + endTime); lblEnd.setStyle("-fx-font-size: 11px; -fx-text-fill: #888888;");
        info.getChildren().addAll(lblName, lblPrice, lblEnd);

        String sc = "ACTIVE".equals(status) ? "#E8F5E9:#27AE60" : "ENDED".equals(status) ? "#F5F5F5:#888888" : "#FCE4EC:#C62828";
        String[] colors = sc.split(":");
        Label lblStatus = new Label("ACTIVE".equals(status) ? "Đang diễn ra" : "ENDED".equals(status) ? "Đã kết thúc" : "Đã hủy");
        lblStatus.setStyle("-fx-background-color: " + colors[0] + "; -fx-text-fill: " + colors[1] + "; -fx-background-radius: 10; -fx-padding: 4 12 4 12; -fx-font-size: 11px; -fx-font-weight: bold;");

        if ("ACTIVE".equals(status)) {
            Button btnEnd = new Button("Kết thúc");
            btnEnd.setStyle("-fx-background-color: #FCE4EC; -fx-text-fill: #C62828; -fx-background-radius: 8; -fx-cursor: hand; -fx-padding: 6 12 6 12;");
